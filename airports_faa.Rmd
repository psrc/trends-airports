...
create.barchart <- function(dataset = c("enplanements", "cargo"), type = c("large", "peer"), year) {
  dt <- compile.faa.hub(dataset, type)[Locid == "SFO", City := "San Francisco"][Locid == "SEA", color := "TRUE"][Locid != "SEA", color := "FALSE"]
  aescoly <- colnames(dt)[str_which(colnames(dt), paste0("^share_cy", year, "-cy2010"))]
  dt[, levels := get(eval(aescoly))][, Locid_sort := factor(Locid, levels = dt$Locid[order(dt$levels)])] # standardized colname for levels to use for factor column
...
g <- create.dotplot("enplanements", "large", 2017) # charts % growth since 2010
print(g)
...
b <- create.barchart("enplanements", "large", 2017) # charts % growth since 2010
print(b)
